...
USBH_ClassTypeDef MIDI_Class = {
    "MIDI", USB_AUDIO_CLASS, USBH_MIDI_InterfaceInit, USBH_MIDI_InterfaceDeInit,
...
static USBH_StatusTypeDef USBH_MIDI_InterfaceInit(USBH_HandleTypeDef *phost) {
  ...
  if (interface == 0xFF) /* No Valid Interface */
  {
    ...
    phost->pActiveClass->pData =
        (MIDI_HandleTypeDef *)USBH_malloc(sizeof(MIDI_HandleTypeDef));
...
USBH_StatusTypeDef USBH_MIDI_Transmit(USBH_HandleTypeDef *phost,
  ...
  if ((MIDI_Handle->state == MIDI_IDLE_STATE) ||
  ...
  switch (MIDI_Handle->data_tx_state) {
    case MIDI_SEND_DATA:
      if (MIDI_Handle->TxDataLength > MIDI_Handle->OutEpSize) {
        USBH_BulkSendData(phost, MIDI_Handle->pTxData, MIDI_Handle->OutEpSize,
                          MIDI_Handle->OutPipe, 1);
      } else {
        USBH_BulkSendData(phost, MIDI_Handle->pTxData,
                          MIDI_Handle->TxDataLength, MIDI_Handle->OutPipe, 1);
      }
...
static void MIDI_ProcessReception(USBH_HandleTypeDef *phost) {
  ...
  switch (MIDI_Handle->data_rx_state) {
    case MIDI_RECEIVE_DATA:
      ...
      USBH_BulkReceiveData(phost, MIDI_Handle->pRxData, MIDI_Handle->InEpSize,
                           MIDI_Handle->InPipe);
    ...
    case MIDI_RECEIVE_DATA_WAIT:
      ...
      if (URB_Status == USBH_URB_DONE) {
        ...
        if (((MIDI_Handle->RxDataLength - length) > 0) &&
            (length > MIDI_Handle->InEpSize)) {
          MIDI_Handle->RxDataLength -= length;
