...
DSP nodes - in this case a [[./src/ex07/sdram_delay.c][new delay node]], which stores its long delay
line in SDRAM, but then has to work with windowed sections in main RAM
and mirror them back.

We also update the DSP node graph to add foldback distortion and
replace the old delay with the new one...
